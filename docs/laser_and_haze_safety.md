...
**Status of this document:** PROPOSED policy. Established on branch
...
current state, which is safer than any of them — by absence rather than by
design.
...
### 2.1 Separate laser master enable
...
- Re-enabling requires manual confirmation again (2.2). Reconnection is **not**
...
### 2.5 Unknown-channel fail-closed behavior
...
### 2.8 Test mode that excludes emission
...
- Recovery requires manual confirmation (2.2).
...
# Part 3 — Haze machine safety and behavior
...
Show generation must place haze cues **ahead of** the moment the effect is
wanted, by a configured per-venue lead time. This is one of the clearest
arguments for offline analysis: the lead time is only computable when the future
...
### 3.9 Section-level control

Haze is an **atmosphere** control operating at the section level: a wash of
haze for a chorus, a clear for a quiet passage, a pre-charge before a drop.

### 3.10 The prohibition

> **Haze must never be mapped directly to beat-level or onset-level audio
> features.**

This is normative, and PD-9 records it as a proposed decision. The reasons:
...
# Part 4 — Venue, regulatory, and operator responsibility
...
| Injectable transports with null implementations | M2 | Test mode (2.8) and the gate's second enforcement point both require it |
...
as PD-4 in [decisions.md](decisions.md): the laser gate must cover the DMX path,
